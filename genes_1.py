...
def get_DR_tris(X, Y, Z=None, T=None, com=2, t1='PCA', t2='tSNE', SaveAs1='PCA', SaveAs2='tSNE', Color1='red', Color2='blue', Label1=None, Label2=None):
    ...
    if com == 2:
        ...
        N1 = np.shape(X)[1]
        N2 = np.shape(Y)[1]
        plt.scatter(y_pca[0:N1,0],y_pca[0:N1,1], c=y_color[0:N1], label=Label1)
        plt.scatter(y_pca[N1:N1+N2,0],y_pca[N1:N1+N2,1], c=y_color[N1:N1+N2], label=Label2)
        ax = plt.gca()
        ...
        plt.savefig(SaveAs1)
        plt.figure()
        plt.scatter(y_tsne[0:N1,0],y_tsne[0:N1,1], c=y_color[0:N1], label=Label1)
        plt.scatter(y_tsne[N1:N1+N2,0],y_tsne[N1:N1+N2,1], c=y_color[N1:N1+N2], label=Label2)
        ax = plt.gca()
        # ax.set_facecolor('white')
        legend = plt.legend(frameon = 1)
        frame = legend.get_frame()
...
def get_DR_Colored_By_User_Provided_Vector(data_matrix, ColorVecotr, MyColorMap, d=2, off=[], t1='PCA', t2='tSNE', SaveAs1='PCA', SaveAs2='tSNE'):
    """
    Dimensionality reduction (pca, tsne, 2 or 3 dimensions)
    g_ind: index of gene used to color plots
    d: dimensions; off: indices of excluded genes (list)
    ...
    if d == 2:
        plt.figure()
        plt.scatter(y_pca[:,0],y_pca[:,1], c=cm, cmap=MyColorMap)
        plt.colorbar()
        plt.title(t1)
        plt.savefig(SaveAs1)
        plt.figure()
        plt.scatter(y_tsne[:,0],y_tsne[:,1], c=cm, cmap=MyColorMap)
        plt.title(t2)
        plt.colorbar()
        plt.savefig(SaveAs2)
    elif d == 3:
        fig = plt.figure()
        ax = fig.add_subplot(111, projection='3d')
        ax.scatter(y_pca[:,0], y_pca[:,1], y_pca[:,2], c=cm, cmap=MyColorMap)
        #plt.colorbar(mappable=cm)
        plt.title(t1)
        fig = plt.figure()
        ax = fig.add_subplot(111, projection='3d')
        ax.scatter(y_tsne[:,0], y_tsne[:,1], y_tsne[:,2], c=cm, cmap=MyColorMap)
        #ax.colorbar()
        plt.title(t2)
    return y_pca, y_tsne
...
class Compare:
    ...
    def merge(self):
        ...
        Additionally, we do not run com_genes befor. Fi c_names is empty it will be run automaticaly"""
